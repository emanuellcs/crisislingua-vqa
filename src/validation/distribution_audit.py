    ...
    def analyze_distribution(self) -> Dict[str, float]:
        ...
        if not self.dataset_file.exists():
            raise FileNotFoundError(
                f"Dataset missing: {self.dataset_file}. Run PII Scrubber first."
            )
    ...
    def validate_compliance(self, distribution: Dict[str, float]):
        ...
        for target in self.TARGET_LANGS:
            ...
            if pct < self.MINIMUM_REPRESENTATION_THRESHOLD:
                logger.warning(
                    f"🚨 CRITICAL: Target language '{target}' is severely underrepresented ({pct:.1%}). "
        ...
        if passed:
            logger.info(
                "🎉 Dataset passes distribution audit. Ready for Kaggle deployment."
            )
        else:
            logger.warning(
                "⚠️ Audit failed. Recommend adjusting heuristic keywords and re-running Phase 1."
            )
