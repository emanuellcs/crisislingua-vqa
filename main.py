...
def run_phase_3() -> bool:
    ...
    auditor = DatasetAuditor(
        dataset_file="data/processed/crisislingua_vqa_sanitized.jsonl"
    )
...
def run_phase_4():
    ...
    try:
        ...
        logger.info(
            "You can now download the dataset directly from the Kaggle notebook output tab."
        )
    except FileNotFoundError:
        logger.error(
            "🚨 Deployment Failed: Source dataset artifact not found. Ensure Phase 3 completed successfully."
        )
        raise
...
def main():
    ...
    try:
        ...
        if is_compliant:
            run_phase_4()
            logger.info(
                "🏆 Hackathon pipeline executed successfully. Ready for submission."
            )
        else:
